...
新たな提案手法：
  Ndepend-SD-CS : SHAP Downsampling with Coverage Scheduling + N依存FPS重み付け
...
- 点の選択は「点レベルのスケジューリング」：
    ...
    λ_cov(N, t, gap) は、
        λ_cov(N, t, gap) = Λ_max(N) · r_FPS(N) · f_t(t/N) · g_gap(gap)
    として設計し、
        - Λ_max(N): N に応じた距離成分の最大スケール（低Nほど大きく、高Nでは小さい）
        - r_FPS(N): N に応じて FPS 距離成分の相対的な重みを調整する係数
                    （N=100〜400 では >1, N=500〜1000 では 1.0 として従来 SD-CS と同等）
...
- Ndepend-SD-CS 固有の Scheduling(N 依存)キャッシュは
...
出力：
- result/NdepSDCS-...(略)_dsSDCS_PointNeXt_h5/{ds_points}/<dataset_subdir>/<test_h5>.h5
...
class AdaptiveGateMLP(torch.nn.Module):
    """
    AMA 用の Gate-MLP。
    出力は (alpha, beta) だが、Ndepend-SD-CS では alpha のみ使用し、
    beta（スケール）は無視する。
    ...
    def forward(self, x: torch.Tensor):
        ...
        beta  = F.softplus(b).squeeze(1)  # 互換性のため残すが Ndepend-SD-CS では使用しない
        return alpha, beta
...
def sdcs_select(points: np.ndarray,
    ...
    """
    Ndepend-SD-CS（点レベル Coverage Scheduling + N依存FPS重み）による点選択アルゴリズム。
...
def main():
    parser = argparse.ArgumentParser(
        description="Ndepend-SD-CS (SD-CS with N-dependent FPS weighting) for ModelNet40 test files using PointNeXt."
    )
    ...
    # 互換性のためのダミー引数（RegionScheduling-SD で使用していたが Ndepend-SD-CS では未使用）
    parser.add_argument("--cov_thresh", type=float, default=0.90,
                        help="(Ndepend-SD-CS では未使用。互換性のために残している)")
    # デバッグ
    parser.add_argument("--debug_sched", action="store_true",
                        help="Ndepend-SD-CS Scheduling の最初の取り出しログを表示する")
    ...
    _out_tag_base = f"NdepSDCS2{l1_tag}-{ama_tag}"
    ...
    # Ndepend-SD-CS Scheduling(N依存)キャッシュ
    BASE_SDCS_CACHE_DIR = os.path.join("result", "_sdcs_cache_global")
    os.makedirs(BASE_SDCS_CACHE_DIR, exist_ok=True)
    sched_cache_tag = f"ndep_sdcs_p{pattern}_kr{k_region}_kp{k_point}_N{ds_points}{('_noL1' if args.no_l1 else '')}_{ama_tag}"
    if args.run_tag:
    ...
    # MID(N非依存)キャッシュ（Ndepend-SD-CS 用に新ディレクトリを用意）
    ...
    for test_file in TEST_FILES:
        ...
        for i in range(num_samples):
            ...
            # =======================
            if use_sched_cache:
                print(f"  ↳ Ndepend-SD-CS scheduling cache hit: {sched_cache_path}")
                ...
                # ==========================
                    ...
                    if pc.shape[0] == 2048:
                        ...
                        # Ndepend-SD-CS Scheduling selection
                        sched0 = time.time()
                        top_indices = sdcs_select(
                            pc, hybrid, ds_points, cov_target_for_N,
                            lambda_scale=args.lambda_scale,
                        ...
                        # MIDキャッシュ保存（Ndepend-SD-CS 用新パスにのみ保存）
                        try:
                ...
                # Ndepend-SD-CS Scheduling結果のキャッシュ保存
        ...
        print(f"Saved Ndepend-SD-CS downsampled point clouds for {test_file} to {output_filename}")
