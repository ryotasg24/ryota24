...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="/workspace/PointNeXt/data/ModelNet40Ply2048/modelnet40_ply_hdf5_2048", help="Path to the modelnet40_ply_hdf5_2048 file")
    # AMA関連
    ...
    if args.ama and args.ama_mode == "mlp" and args.gate_ckpt:
        ...
        if args.ama_mode == "mlp" and not args.gate_ckpt:
            print("[WARN] --ama_mode=mlp ですが --gate_ckpt が未指定です。heuristic 相当の挙動になります。")
    ...
    out_tag = f"AMA-{ama_tag}" + (f"-{gate_tag}" if (ama_tag == "mlp" and gate_tag) else "")
    output_folder = os.path.join(
        "result",
        f"{out_tag}_kr{k_region}_kp{k_point}_p{pattern}_dsSHAP_PointNeXt_h5",
        str(ds_points)
    )                    ####出力ファイルパスの修正
    ...
    if args.gate_dump_train:
        # 設定で分離（例: result/_gate_train_dump/p10000_kr32_kp64_heuristic/）
        dump_tag = f"p{pattern}_kr{k_region}_kp{k_point}_{('mlp' if args.ama_mode=='mlp' else 'heuristic')}"
    ...
    cache_tag = f"p{pattern}_kr{k_region}_kp{k_point}_ama-{ama_tag}_N{ds_points}"
    ...
    # === N非依存の中間キャッシュ (Region/Point per-point) ===
    MID_CACHE_BASE = os.path.join("result", "_shap_cache_mid")
    # モデル依存なので ckpt のベース名もタグに含める
    ckpt_name_for_tag = Path(args.checkpoint_path).stem
    mid_tag = f"p{pattern}_kr{k_region}_kp{k_point}_ckpt-{ckpt_name_for_tag}"
    ...
    for test_file in TEST_FILES:
        ...
        for i in range(num_samples):
            ...
            if use_cache:
                ...
                top_indices    = sorted_indices[:ds_points]
                ds_pc          = pc[top_indices, :]
                # ついでに Nキャッシュ（完成形）も書いておくと更に次回が速い
                if save_cache:
            ...
            # ==========================================================
                ...
                if pc.shape[0] == 2048:
                    ...
                    def shap_predict_A(mv):
                        out = shap_predict_block_mask(mv, reg_A, pc_A, baseline_reg_A, model, device, chunk_size=args.mask_chunk, pin_mem=(not args.no_pinmem), min_chunk_size=args.min_mask_chunk)
                        preds_A.append(out)
                        return out
                    ...
                    def shap_predict_B(mv):
                        out = shap_predict_block_mask(mv, reg_B, pc_B, baseline_reg_B, model, device, chunk_size=args.mask_chunk, pin_mem=(not args.no_pinmem), min_chunk_size=args.min_mask_chunk)
                        preds_B.append(out)
                        return out
                    ...
                    # === MID（N非依存）保存用の共通変数をセット ===
                    mid_region = np.concatenate([region_A_per_pt, region_B_per_pt], axis=0).astype(np.float32)
                    mid_point  = np.concatenate([pt_adj_A,       pt_adj_B],       axis=0).astype(np.float32)
                    mid_pc     = np.concatenate([pc_A, pc_B], axis=0).astype(np.float32)
                    h1 = time.time()
                    ...
                    def shap_predict_blk(mv):
                        out = shap_predict_block_mask(mv, reg_blk, pc, baseline_reg_blk, model, device, chunk_size=args.mask_chunk, pin_mem=(not args.no_pinmem), min_chunk_size=args.min_mask_chunk)
                    ...
                    # === MID（N非依存）保存用の共通変数をセット ===
                    mid_region = region_blk_per_pt.astype(np.float32)
                    mid_point  = pt_adj_blk.astype(np.float32)
                    mid_pc     = pc.astype(np.float32)
                    h1 = time.time()
                ...
                # --- MIDキャッシュ保存（N非依存；常に保存） ---
                try:
                    np.savez_compressed(
                        mid_cache_path,
                        region_per_pt=mid_region,
                        point_adj=mid_point,
                        pc=mid_pc,
                        label=np.array([cls], dtype=np.int32)
                    )
