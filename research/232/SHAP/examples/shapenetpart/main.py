...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    with np.printoptions(precision=2, suppress=True):
        logging.info(f'Test Epoch [train-epoch:{cfg.epochs}],\n'
                        f'Instance mIoU {ins_miou:.2f}, '
                        f'Class mIoU {cls_miou:.2f}, '
                        f'\n Class mIoUs {cls_mious}')
        # ShapeNet Part では data_root/…/synsetoffset2category.txt にマッピングがある
        root = cfg.dataset.common.data_root if hasattr(cfg.dataset, 'common') \
            else cfg.dataset.root

        # root がすでにフォルダ名で終わっているかを確認して重複を防ぐ
        if not root.endswith('shapenetcore_partanno_segmentation_benchmark_v0_normal'):
            root = os.path.join(root, 'shapenetcore_partanno_segmentation_benchmark_v0_normal')

        map_file = os.path.join(root, 'synsetoffset2category.txt')

        # 例: 02691156 airplane のように <name>\t<synset> が並ぶ
        id2name = {}
        with open(map_file, 'r') as f:
            for line in f:
                name, syn = line.strip().split()
                id2name[syn] = name.capitalize()          # Airplane, Bag …

        # dataloader 側で保持している synset → index の順に並べ替え
        if hasattr(val_loader.dataset, 'cat2id'):
            # cat2id: synset → idx  の dict （OpenPoints の ShapeNet 実装が持っている）
            idx2name = [''] * len(val_loader.dataset.cat2id)
            for syn, idx in val_loader.dataset.cat2id.items():
                idx2name[idx] = id2name[syn]
        else:
            # フォールバック：事前に定義してあった並びに合わせる
            idx2name = [id2name[syn] for syn in sorted(id2name)][:cfg.shape_classes]

        # cls_mious は 0〜100 の tensor。高い順に並べ替えて表示
        per_class = sorted(
            [(idx2name[i], float(cls_mious[i])) for i in range(cfg.shape_classes)],
            key=lambda x: x[1],
            reverse=True)

        table = '\n'.join(f'{name:>10}: {miou:5.2f} %' for name, miou in per_class)
        logging.info('\nPer-class mIoU (high → low)\n' + table)
