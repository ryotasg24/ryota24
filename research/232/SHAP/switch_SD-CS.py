...
出力：
- result/switchSDCS-...(略)_dsSDCS_PointNeXt_h5/{ds_points}/<dataset_subdir>/<test_h5>.h5
...
新たな提案手法：
    switch-SD-CS : SHAP Downsampling with Coverage Scheduling + N依存FPS重み付け

修正箇所：スコア式およびその呼び出し（score_i(t,N) に Hybrid 重み w_H(N) を導入）、
          また SD-CS の各項（被覆率・FPS距離項）の ON/OFF フラグを追加。
...
- 点の選択は「点レベルのスケジューリング」：
    ...
    スコア：
        score_i(t, N) = w_H(N) · Hybrid[i]
                        + λ_cov(N, t, gap_cov(t, N)) · d_i(t)

    ここで
        - w_H(N) は Hybrid-SHAP の寄与を抑制/強調する N依存重み。
          初期設定として、
            N=100〜400 : w_H(N)=0 （Hybrid項を無効化し、FPS距離項のみでスコアリング）
            N=500〜1000: w_H(N)=1 （Hybrid項をフルに使用）
          とし、それ以外の N では N<500→0, N>=500→1 とする。
        - λ_cov(N, t, gap) は、Coverage Scheduling + N依存FPS重み付けを行う係数。
          Coverage gap (被覆率不足) と FPS距離項を組み合わせて、
          Hybrid と距離項のバランスを N, t, coverage に応じて制御する。
        - 本コードではさらに、被覆率を用いるかどうか（g_gap の ON/OFF）、
          FPS距離項を使うかどうか（λ_cov·d_i の ON/OFF）を
          コマンドライン引数で切り替えられるようにしている。
...
アルゴリズム概要：
  ...
  2. ダウンサンプリングで N 点取りたいとき、
     ...
     ステップを t = 1〜N 回繰り返す。
  3. t=1 では Hybrid-SHAP のみ、あるいは w_H(N)=0 の場合は FPS にならうため
     ランダムに 1 点を選択する（FPS の1点目がランダムであることに対応）。
  4. t>=2 では、
...
- switch-SD-CS 固有の Scheduling(N 依存)キャッシュは
      result/_sdcs_cache_global/...
  とし、従来の Scheduling-SD の SCHED キャッシュとは分離する。
- 本バージョンでは、
    * Hybrid 項の N依存重み w_H(N)
    * Coverage gap に基づく項（被覆率）
    * FPS距離項（λ_cov·d_i）
  をそれぞれ制御できるようにし、「何が効いているか」をアブレーションで確認できる。
"""
...
class AdaptiveGateMLP(torch.nn.Module):
    """
    AMA 用の Gate-MLP。
    出力は (alpha, beta) だが、switch-SD-CS では alpha のみ使用し、
    beta（スケール）は無視する。
    ...
    def forward(self, x: torch.Tensor):
        ...
        beta  = F.softplus(b).squeeze(1)  # 互換性のため残すが switch-SD-CS では使用しない
        return alpha, beta
...
def get_r_fps(N: int) -> float:
    """
    N に依存して FPS 距離項の相対重み r_FPS(N) を返す（強め版）。

    - N <= 100      : r_FPS = 2.5
    - 100 < N <=400 : r_FPS を 2.5 → 1.5 に線形に減少
    - 400 < N < 500 : r_FPS を 1.5 → 1.0 に線形に減少
    - 500 <= N      : r_FPS = 1.0 （従来 SD-CS と同等）
    """
    N = int(N)
    if N <= 100:
        return 2.5
    if N <= 400:
        # 2.5 → 1.5 を 100〜400 で線形に減少
        val = 2.5 - 1.0 * (N - 100) / 300.0
        return float(val)
    if N < 500:
        # 1.5 → 1.0 を 400〜500 で線形に減少
        val = 1.5 - 0.5 * (N - 400) / 100.0
        return float(val)
    # 500〜1000, それ以上は 1.0
    return 1.0


def get_hybrid_weight_for_N(N: int) -> float:
    """
    Hybrid-SHAP の項に掛ける N依存の重み w_H(N)。

    FPS と SHAP のスイッチング用途として、
      - N=100〜400 : 0.0 （Hybrid項を無効化し、FPS距離項のみでスコアリング）
      - N=500〜1000: 1.0 （Hybrid項をフルに使用）
    とする。
...
def compute_lambda_cov(N: int,
    ...
    ・r_FPS(N):
        N=100〜400 では 1.0 より大きく設定し、低Nほど FPS 距離成分を相対的に強める。
        N>=500      では 1.0 とし、従来 SD-CS と同等の挙動にする。
    ...
    ・g_gap(gap):
        ...
        use_cov_gap=False の場合は常に 1.0 とし、被覆率に依存しない。
    """
    if N <= 0:
        return 0.0
...
def sdcs_select(points: np.ndarray,
                ...
                debug: bool = False,
                hy_weight: float = 1.0,
                use_cov_gap: bool = True,
                use_fps_term: bool = True) -> np.ndarray:
    """
    switch-SD-CS（点レベル Coverage Scheduling + N依存FPS重み）による点選択アルゴリズム。
    ...
    引数：
        ...
        hy_weight     : w_H(N)。score_i に対して w_H(N)·Hybrid[i] を掛ける。
        use_cov_gap   : Coverage gap による g_gap を有効にするかどうか。
        use_fps_term  : FPS距離項 λ_cov·d_i をスコアに入れるかどうか。

    スコア：
        step>=1 において
            score_i = w_H(N)·Hybrid[i] + λ_cov·d_i
        step=0 では、w_H(N)>0 のとき w_H(N)·Hybrid による1点選択、
                  w_H(N)=0 のとき FPS に倣ってランダムに1点選択する。
    """
    ...
    for step in range(target_N):
        # step=0: Hybrid or Random（w_H(N)=0 のとき FPS の1点目に倣いランダム）
        if step == 0:
            if hy_weight != 0.0:
                scores = hy_weight * hybrid_scores.copy()
                ...
                # Hybrid を使わず、FPS に倣ってランダムに1点目を選択
                available_indices = np.where(~selected_flag)[0]
                if len(available_indices) == 0:
                    chosen_idx = 0
                else:
                    chosen_idx = int(np.random.choice(available_indices))
            ...
            if debug:
                ...
                print(f"[DEBUG_SDCS] step={step:03d}, pid={chosen_idx:04d}, "
                      ...
                      f"hy_w={hy_weight:.3f}, coverage={cov_now:.4f}, "
                      f"lambda=0.0000, d_norm=0.0000")
            continue
        ...
        lam = compute_lambda_cov(
            ...
            use_cov_gap=use_cov_gap
        )

        if not use_fps_term:
            lam = 0.0

        scores = hy_weight * hybrid_scores + lam * d_norm
        scores[selected_flag] = -np.inf
        ...
        if debug and step < 40:
            ...
            print(f"[DEBUG_SDCS] step={step:03d}, pid={chosen_idx:04d}, "
                  f"hybrid={float(hybrid_scores[chosen_idx]):.6f}, "
                  f"hy_w={hy_weight:.3f}, coverage={cov_now:.4f}, "
                  f"lambda={lam:.4f}, d_norm={d_chosen:.4f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="switch-SD-CS (SD-CS with N-dependent FPS weighting) for ModelNet40 test files using PointNeXt."
    )
    ...
    # SD-CS 各項のON/OFF（アブレーション用）
    parser.add_argument("--disable_cov", action="store_true",
                        help="Coverage gap による g_gap を無効化する（被覆率に依存しない λ_cov にする）")
    parser.add_argument("--disable_fps", action="store_true",
                        help="FPS距離項 λ_cov·d_i を無効化する（Hybrid のみのスコアにする）")
    # 互換性のためのダミー引数（RegionScheduling-SD で使用していたが switch-SD-CS では未使用）
    parser.add_argument("--cov_thresh", type=float, default=0.90,
                        help="(switch-SD-CS では未使用。互換性のために残している)")
    # デバッグ
    parser.add_argument("--debug_sched", action="store_true",
                        help="switch-SD-CS Scheduling の最初の取り出しログを表示する")
    ...
    _out_tag_base = f"switchSDCS{l1_tag}-{ama_tag}"
    ...
    # switch-SD-CS Scheduling(N依存)キャッシュ
    BASE_SDCS_CACHE_DIR = os.path.join("result", "_sdcs_cache_global")
    os.makedirs(BASE_SDCS_CACHE_DIR, exist_ok=True)
    # スコア式を変更したので、タグの先頭を ndep_sdcs2_ にして旧バージョンと分離
    sched_cache_tag = f"ndep_sdcs2_p{pattern}_kr{k_region}_kp{k_point}_N{ds_points}{('_noL1' if args.no_l1 else '')}_{ama_tag}"
    if args.disable_cov:
        sched_cache_tag += "_noCov"
    if args.disable_fps:
        sched_cache_tag += "_noFPS"
    if args.run_tag:
    ...
    # MID(N非依存)キャッシュ（switch-SD-CS 用に新ディレクトリを用意）
    ...
    print(f"[INFO] r_FPS(N={ds_points}) = {get_r_fps(ds_points):.4f}")

    hybrid_weight_for_N = get_hybrid_weight_for_N(ds_points)
    print(f"[INFO] hybrid_weight w_H(N={ds_points}) = {hybrid_weight_for_N:.3f}")
    print(f"[INFO] use_coverage_gap = {not args.disable_cov}, use_fps_term = {not args.disable_fps}")
    ...
    for test_file in TEST_FILES:
        ...
        for i in range(num_samples):
            ...
            # =======================
            if use_sched_cache:
                print(f"  ↳ switch-SD-CS scheduling cache hit: {sched_cache_path}")
                ...
                if mid_hit and (not args.force_mid_build):
                    ...
                    top_indices = sdcs_select(
                        ...
                        hy_weight=hybrid_weight_for_N,
                        use_cov_gap=not args.disable_cov,
                        use_fps_term=not args.disable_fps
                    )
                    sched1 = time.time()
                ...
                # ==========================
                    ...
                    if pc.shape[0] == 2048:
                        ...
                        # switch-SD-CS Scheduling selection
                        sched0 = time.time()
                        top_indices = sdcs_select(
                            pc, hybrid, ds_points, cov_target_for_N,
                            lambda_scale=args.lambda_scale,
                        ...
                        # MIDキャッシュ保存（switch-SD-CS 用新パスにのみ保存）
                        try:
                ...
                # switch-SD-CS Scheduling結果のキャッシュ保存
        ...
        print(f"Saved switch-SD-CS downsampled point clouds for {test_file} to {output_filename}")
